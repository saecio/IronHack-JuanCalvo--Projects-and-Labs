...
SELECT 
    ...
    DAYOFWEEK(rental_date) AS wday_rental,
    inventory_id,
    customer_id,
    return_date,
    MONTH(return_date) AS month_return,
    DAYOFWEEK(return_date) AS wday_return,
    staff_id,
    last_update
FROM
    rental
...
SELECT 
    ...
    CASE
        WHEN
            DAYOFWEEK(rental_date) = 1 OR DAYOFWEEK(rental_date) = 7
        THEN
            'weekend'
        ELSE 'workday'
    END AS rental_date_cat,
    inventory_id,
    customer_id,
    return_date,
    MONTH(return_date) AS month_return,
    ...
    CASE
        WHEN
            DAYOFWEEK(return_date) = 1 OR DAYOFWEEK(return_date) = 7
        THEN
            'weekend'
        ELSE 'workday'
    END AS return_date_cat,
    staff_id,
    last_update
FROM
    rental
...
SELECT 
    ...
    first_name,
    LEFT(email, 3) AS email_first_3_chars,
    email
FROM
    customer
ORDER BY last_name;
...
SELECT 
    COUNT(DISTINCT film_id) AS num_films_released
FROM
    film;

-- 1.2 The number of films for each rating.
SELECT 
    rating, COUNT(DISTINCT film_id) AS num_films
FROM
    film
GROUP BY rating;
